...
\section{Implementation} \label{implementation}
    ...
    The scheduler class takes care of the multiprocessing between reading the
    input, calculating on the GPU, and writing the output. It acts as the 
    manager of an assembly line to make sure parts are processed in the correct
    order and no action is taken unless it is prompted to in order to produce 
    the correct product.
    \subsection{Loader}
    This loader class reads the data from a given input and passes it into a
    pipe object. The input can come in as GEOtiff \(.tif\) or an ASCII \(.asc\)
    format selected from the disk or from QGIS when a raster image is already 
    loaded. The header info for the data is stored for the output and the
    data it self is put into the pipe lines at a time to be worked.
    \subsection{GPU Manager}
    The GPU manager is where the desired slope, aspect, or hillshade calculations
    are done. This class takes the data from the pipe that the loader added and 
    spreads out over the GPUs many cores to perform its many calculations nearly 
    simultaniously. The results are then places into the second pipe that leads 
    to the saver to be written on to the disk.
    \subsection{Saver}
    The data saver class takes the results sent throguh the pipe by the GPU
    manager and saves it to the disk. It takes the result and saves as a GEOtiff 
    \(.tif\) file in order to conserve time and file size. 
    \subsection{QGIS Plugin}
    The QGIS plugin lays out the gui for this implementation and lets our 
    program be used from the app. The plugin loads a pop up which allows the user
    to specify their input and where tosave their output. The user then selects 
    the combination of slope, aspect, and/or hillsahde they want calculated as 
    well if they want ouput loaded directly into Qgis for viewing. While the
    program is running a loading bar is visable to update on progress.
...
output are the biggest hurdles to overcome. Timing data can be seen in these
tables.
...
\begin{tabular}{ l c r }
    File Size & QGIS   & PyCUDA\\
    25 MB     & 5 secs & 3 secs\\
    200 Mb    & 15 secs & 6 secs\\
    1.5 GB    & 17 mins & 6 mins\\
    8 GB      & 1:30+ hrs & 45 mins
\end {tabular}
...
\begin{tabular}{ l c r }
    Function & QGIS   & PyCUDA\\
    Input    & 2:30 & 6:00\\
    Computation  & 6:00 & 3:48\\
    Output    & 5:00 & 2:15\\
    Total   & 14:00 & 6:10
\end {tabular}
...
\begin{tabular}{ l c r }
    Function & QGIS   & PyCUDA\\
    Input    & 2:30 & 6:00\\
    Computation  & 9:00 & 3:48\\
    Output    & 5:00 & 2:15\\
    Total   & 17:00 & 6:10
\end {tabular}
\vspace{0.25in}

The PyCUDA version is consistently faster than QGIS when calculating hllshade for files
of various sizes. The I/O bottleneck  can be seen in the input and output sections 
of the second table. PyCUDA spends as much time dealing with disk as QGIS does which
limits the possible speedup from using the GPU. Input takes a much longer time because
it has to wait for the GPU to pass its data to the saver before it can start sending 
information. Output is slightly faster thanks to compression done by GDAL.
GPU computations, including CPU based memory management and took one third of the time 
required to do the same thing in QGIS. While not shown in the tables, the total time 
just on the GPU to compute hillshade for an 8 GB file is incredibly small.
Including moving data to and from the card, the accumulated execution took less 
than 2 seconds. Both slope and hillshade are calculated in the same time by the GPU, showing that 
it isn't being taxed by the more complicated formula and could easily do more work.
A processs that required hundreds or thousands of computations per element would be
perfect for adaptation onto GPU processing. Testing showed that even computing 
hillshade several hundred times on each pixel didn't increase the time needed to finish.
...
utilizes multithreading so that I/O and GPU computation can occur
...
implementations 2-3x faster than the serial ones typically used by QGIS, and
...
as analysis on raster data where each computation may have depenency on
separate raster data, or even analysis on vector data.
...
improve I/O may lead to more complex code that is both more difficult to read,
