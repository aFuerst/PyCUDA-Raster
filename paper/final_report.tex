...
\section{Methods} \label{methods}
    ...
    The algorithms we are implementing utilize a 3x3 cell neighborhood around each 
    calculated cell. 
    \begin{figure}
        \center
...
\section{Implementation} \label{implementation}
    ...
    The scheduler class starts and manages the other processes. It is responsible
    for parsing the desired input, functions, and output. It starts each other
    process and coordinates passing information between them. The scheduler also
    ensures that each other process terminates properly, and if not attempts to print
    useful information about which process caused the error.
    \subsection{Loader}
    This loader class reads the data from a given input file and passes it into
    a pipe object, which goes to the GPU manager. We use the GDAL libraries to
    read files from disk because it optimized on raster data files. The data is
    ...
    because fewer system calls are made that way. Empirically, we found that it
    was optimal to cache around 30 rows at a time to be sent through the pipe.
    ...
    simultaneously. The results are then places into the second pipe that leads 
    to the saver to be written on to the disk.
    \subsection{Saver}
    The data saver class takes the results sent through the pipe by the GPU
    ...
    to specify their input and where to save their output. The user then selects 
    the combination of slope, aspect, and/or hillsahde they want calculated as 
    well if they want output loaded directly into QGIS for viewing. While the
    program is running a loading bar is visible to update on progress.
...
\begin{tabular}{ l | c | c }
    ...
    Input       & 2:30 & 6:00\\ \hline
    Computation & 6:00 & 3:48\\ \hline
    Output      & 5:00 & 2:15\\ \hline
    Total       & 14:00 & 6:10
\end {tabular}
...
\begin{tabular}{ l | c | c }
    Function    & QGIS & PyCUDA\\ \hline
    Input       & 2:30 & 6:00\\ \hline
    Computation & 9:00 & 3:48\\ \hline
    Output      & 5:00 & 2:15\\ \hline
    Total       & 17:00 & 6:10
\end {tabular}
\vspace{0.25in}

The PyCUDA version is consistently faster than QGIS when calculating hillshade for files
...
limits the possible speedup from using the GPU. Output is slightly fast thanks to compression
done by GDAL. This speed increase is still significant however. GPU computations, 
including CPU based memory management and took one third of the time 
...
than 2 seconds.  A processs that required hundreds or thousands
of computations per element would be perfect for adaptation onto GPU processing.
Both slope and hillshade are calculated in the same time by the GPU, showing that 
