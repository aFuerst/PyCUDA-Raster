...
Similar work on creating open source, accessible parallel GIS tools has been
done in the past, though no one method has been widely adopted. The authors of
\cite{guan} created a C++ library for parallel raster file processing, though
this only uses CPU parallelization. \cite{benedicic} created a GRASS GIS module
based on cluster computing designed to predict radio-coverage.  \cite{hpc_cuda}
developed a C++ library for simple raster calculations using CUDA as an
extension to the proprietary ArcGIS software. We want to progress the work
of \cite{hpc_cuda} to be more accessible to the GIS community.

\section{Problem Outline}
GIS analytics perform many independent calculations on extremely large data
files. This poses the significant issues of memory limitations and time to
complete. Data files typically analyzed by GIS now can be in the terabyte (TB)
size realm. With most computers only having memory capabilities in the gigabyte
(GB) range, steps need to be taken to work around running out of memory. Both
the large size of files the requirement that each piece of data be analyzed
separately slows down calculations dramatically. Input from and output to
data files and memory have proven to be a major component to the slow down.
However performing the large amount of independent calculations through serial
programming also bogs down the process. Our goal is to exploit the maximum amount
of memory on the machine without over reaching and simultaneously run parallel
calculations to minimize computation time.

\section{Methods}
Our implementation of this plugin utilizes parallelism both on the GPU and its
host computer. It contains a kernel written in C that the CUDA compiler will
use when it comes time to perform calculations on the  GPU. This kernel will be
...
\section{Analysis}
Timing our implementation for parallel slope calculation from a raster file against the standard 
serial method used in QGIS has revealed the input and output will be our biggest  hurdle to 
overcome. Testing with a 23 megabyte (MB) data file our implementation completed on 
average in nine seconds while the QGIS method finishes on average five seconds. Moving to a 200 
megabyte (MB) file the time difference increased with QGIS completing in 15 seconds and our
version finishing in 51 seconds. Through testing these files though the calculation time on our version 
appears almost immediately after reading the file while calculations in QGIS are noticeably taking time
to complete. Moving on to even larger file sizes, in the gigabyte (GB) range, is where conflicts 
arose. The next file size up was a 3.1 gigabyte (GB) file and our implementation failed with an 
illegal memory access. The same happened with the largest test file we have, which is 8 gigabytes (GB).
These files do finish in QGIS in 10 minutes and 27 minutes respectively. This failure is most likely 
the result of running out of memory on our GPU and something to correct in the future along 
with optimizing input and output for max efficiency.
